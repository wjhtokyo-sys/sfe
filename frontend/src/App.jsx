...
function ItemAdminPanel({ items, kw, setKw, load, authHeaders }) {
  ...
  return <Card className='panel' title='商品信息管理'>
    ...
    <Space wrap style={{ marginTop: 8 }}>
      ...
      <Space direction='vertical' size={2}><span>建议价</span><InputNumber min={0} placeholder='建议价' value={newItem.msrp_price} onChange={(v) => setNewItem({ ...newItem, msrp_price: v || 0 })} /></Space>
      <Space direction='vertical' size={2}><span>入数</span><InputNumber min={1} placeholder='入数' value={newItem.in_qty} onChange={(v) => setNewItem({ ...newItem, in_qty: v || 1 })} /></Space>
