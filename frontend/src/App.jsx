...
export default function App() {
  ...
  const orderCols = [{ title: '订单ID', dataIndex: 'id' }, { title: role === 'super_admin' ? '客户名' : '客户ID', dataIndex: 'customer_id', render: (v) => role === 'super_admin' ? (data.customers.find(c => c.id === v)?.name || `客户${v}`) : v }, { title: '商品', dataIndex: 'item_name_snapshot' }, { title: '订货', dataIndex: 'qty_requested' }, { title: '已分配', dataIndex: 'qty_allocated' }, ...(role === 'super_admin' ? [{ title: '状态', dataIndex: 'status', render: (v) => <Tag>{v}</Tag> }] : [])];
